## nohup R CMD BATCH --no-save --no-restore fb_30days_regions_import.R  /home/poleinikov/FB_reports/Logs/30days_import_$(date +%Y-%m-%d).txt &

library(readr)
library(dplyr)
library(RMySQL)

conn = dbConnect(RMySQL::MySQL(), host="xxx.xxx.xx.xx", ## replace with IP address or 'localhost'
                 user="xxxx", password="xxxx",
                 dbname="dbase1")

d = dbGetQuery(conn, "select distinct date from fb_30days_region")

data_files = list.files(path="./FB_reports/30Days", 
                        pattern="FacebookAdLibraryReport_.*_US_last_30_days.zip",
                        full.names = T)
...
## the first date when we started saving 'Last 30 Days' reports was 02/09/2020
...
if (nrow(df) > 0) {
  for (j in 1:nrow(df)) {
    ...
    data_files = list.files(path="./FB_reports/tmp/regions", 
                            pattern="FacebookAdLibraryReport.+last_30_days_.+.csv",
                            full.names = T)
    ...
    for (k in 1:nrow(df_region)) {
      ...
      d = read_csv(file=I(tmp_text), guess_max=100000, col_types="cccc?")
    ...
    dbWriteTable(conn, name="fb_30days_region", value=d2,
