## nohup R CMD BATCH --no-save --no-restore fb_daily_regions_import.R  /home/poleinikov/FB_reports/Logs/regions_import_$(date +%Y-%m-%d).txt &

library(readr)
library(dplyr)
library(RMySQL)

conn = dbConnect(RMySQL::MySQL(), host="xxx.xxx.xx.xx", ## replace host with IP address or 'localhost'
                 user="xxxx", password="xxxx",
                 dbname="dbase1")

d = dbGetQuery(conn, "select distinct date from fb_daily_region")
...
## post Georgia run-off elections
...
if (nrow(df) > 0) {
  for (j in 1:nrow(df)) {
    ...
    data_files = list.files(path="./FB_reports/tmp/regions", 
                            pattern="FacebookAdLibraryReport.+yesterday_.+.csv",
                            full.names = T)
    ...
    dbWriteTable(conn, name="fb_daily_region", value=d2,
