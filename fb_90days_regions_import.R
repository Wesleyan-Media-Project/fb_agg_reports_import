## nohup R CMD BATCH --no-save --no-restore fb_90days_regions_import.R  /home/poleinikov/FB_reports/Logs/90days_import_$(date +%Y-%m-%d).txt &
...
d = dbGetQuery(conn, "select distinct date from fb_90days_region")

data_files = list.files(path="/home/poleinikov/FB_reports/90Days", 
                        pattern="FacebookAdLibraryReport_.*_US_last_90_days.zip",
                        full.names = T)
...
df = df %>% filter(report_date >= "2020-02-09")
...
if (nrow(df) > 0) {
  for (j in 1:nrow(df)) {
    ...
    data_files = list.files(path="~/FB_reports/tmp/regions", 
                            pattern="FacebookAdLibraryReport.+last_90_days_.+.csv",
                            full.names = T)
    ...
    dbWriteTable(conn, name="fb_90days_region", value=d2,
